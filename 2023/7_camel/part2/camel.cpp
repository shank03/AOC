...
std::vector<std::string> possible_combinations(std::string hnd) {
    if (hnd.empty()) {
        return { "" };
    }

    std::vector<std::string> res;
    std::string              available_cards = (hnd[0] == 'J') ? "23456789TQKA" : std::string(1, hnd[0]);
    for (char x : available_cards) {
        std::vector<std::string> rest = possible_combinations(hnd.substr(1));
        for (std::string y : rest) {
            res.push_back(x + y);
        }
    }
    return res;
...
Hand extract_hand(std::string &str) {
    ...
    std::vector<std::string> comb = possible_combinations(hnd.hand);
    for (auto &c : comb) {
        hnd.type = std::max(hnd.type, validate_hand(c));
    }
    return hnd;
}
