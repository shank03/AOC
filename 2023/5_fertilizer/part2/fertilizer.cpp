...
struct Range {
    ...
    bool validate_range(Range *r, std::vector<Range> *list) {
        uint64_t s = std::max(r->src, src);
        uint64_t e = std::min(r->dst, src + offset);
        if (s < e) {
            if (s > r->src) {
                list->push_back(Range { .src = r->src, .dst = s });
            }
            if (e > r->dst) {
                list->push_back(Range { .src = e, .dst = r->dst });
            }
            r->src = s - src + dst;
            r->dst = e - src + dst;
            return true;
...
std::vector<Range> extract_seeds(std::string &str) {
    std::string              seeds_data = split(str, "seeds: ")[1];
    std::vector<std::string> seeds      = split(seeds_data, " ");

    std::vector<Range> res;
    for (int i = 0; i < seeds.size(); i += 2) {
        uint64_t s = uint64_t(std::atoi(seeds[i].c_str()));
        uint64_t r = uint64_t(std::atoi(seeds[i + 1].c_str()));
        res.push_back(Range { .src = s, .dst = s + r, .offset = r });
    }
    return res;
}
...
void map_range(Element el_src, Element el_dst, std::map<Element, std::vector<Range>> *elements, Map &map) {
    std::vector<Range> src_el = (*elements)[el_src];
    for (int i = 0; i < src_el.size(); i++) {
        Range s     = src_el[i];
        bool  found = false;
        for (auto &r : map.ranges) {
            Range v = s;
            if ((found = r.validate_range(&v, &src_el))) {
...
uint64_t solve(std::vector<std::string> &lines) {
    ...
    std::map<Element, std::vector<Range>> elements;
    ...
    for (auto &l : elements[Element::LOCATION]) {
        res = std::min(res, l.src);
        res = std::min(res, l.dst);
    }
    return res;
}
